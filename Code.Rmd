...
From the boxplots for NDAI based on different labels, we can clearly see that the NDAI is higher for cloud pixels than those cloud-free ones. The distrubition for pixels labeled as clouds are left skewed, while the distribution for cloud-free pixels are roughly symmetric. The distribution for three different image data are roughly the same as well.
...
det_x = (max_x - min_x) / 5
det_y = (max_y - min_y) / 5
x_cut = seq(min_x, max_x + 1, det_x)
y_cut = seq(min_y, max_y + 1, det_y)
x_cut
